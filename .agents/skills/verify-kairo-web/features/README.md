...
| `console-ref-notes.md` | #411 Console：Ref 详情 notes（digest 之上）列表/只读/空态/轻量追加；阅读画布与详情同一套文档批注观感；单条详情确认删除；Topic 形态整行进 `#reader` | `/w/{slug}` 形态与 `#reader`、`/refs/{id}`、`/refs/{id}/notes/{note_id}`。LLM / step / compose **skip** |
