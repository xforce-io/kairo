...
def test_audio_and_document_roles_unchanged():
    ...
    for ext in (".mp4", ".m4v", ".mov", ".webm"):
        assert rbe[ext] == "audio", ext
    assert rbe[".pdf"] == "document"


def test_guess_role_falls_back_to_builtin_for_old_workspace(tmp_path):
    # 旧 workspace 的 constitution 冻结了无图片/视频扩展名的 roles_by_ext;
    # guess_role 应回退内置默认 → .jpg 仍归 attachment、.mp4 仍归 audio(不落到 transcript)
    import yaml
    from kairo.workspace import Workspace
    ws = Workspace.init(tmp_path / "ws", topic="t")
    cpath = ws.root / "constitution.yaml"
    data = yaml.safe_load(cpath.read_text())
    data["roles_by_ext"] = {".m4a": "audio", ".pdf": "document"}  # 模拟旧配置(无图片/视频)
    ...
    assert ws.guess_role(Path("x.mp4")) == "audio"
    assert ws.guess_role(Path("x.mov")) == "audio"
