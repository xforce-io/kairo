...
def test_glossary_parsed_from_yaml():
    ...
    assert [e.name for e in con.glossary] == ["康医通", "陈伟"]
    assert con.glossary[0].aka == ["看医通", "看一通"]
...
def test_glossary_reference_contains_name_note_aka():
    con = Constitution(
        glossary=[
            GlossaryEntry(name="康医通", note="平台正式名", aka=["看医通", "看一通"]),
            GlossaryEntry(name="陈伟", note="协和营养科主任"),
        ]
    )
    block = con.glossary_reference()
    # 真名是主角:都要出现
    assert "康医通" in block and "陈伟" in block
    # note 给模型 grounding
    assert "平台正式名" in block
    # aka 作参考变体出现
    assert "看医通" in block and "看一通" in block
...
def test_glossary_injected_into_digest_and_compose(tmp_path):
    ...
    assert "康医通" in digest and "规范名" in digest
    assert "康医通" in understanding
