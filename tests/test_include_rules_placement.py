...
def _topic(tmp_path):
    serve = tmp_path / "root"
    serve.mkdir()
    Workspace.init(serve / "energy", topic="北港梳理")
    create_tag(serve, "流程质量")
    create_tag(serve, "北港梳理")
    set_include_tags(serve, "energy", ["北港梳理"])
    return serve
...
def test_include_tags_form_post_still_saves(tmp_path):
    ...
    assert include_tags_of(Workspace.open(serve / "energy")) == ["北港梳理", "流程质量"]
