...
> **测试运行约定(重要):** 所有需要 stub 的测试都用 `monkeypatch.setenv("KAIRO_STUB","1")` 自管环境。**不要**在命令行全局加 `KAIRO_STUB=1` 跑整套/多文件——那会把本应真跑的 markitdown(`tests/test_engine.py` 的二进制链测试)也 stub 掉,造成假失败。统一用 `uv run pytest ...`(不带全局 env 前缀)。
...
Run: `uv run pytest tests/test_engine_pending.py -v`
...
Run: `uv run pytest tests/test_engine_pending.py tests/test_engine.py tests/test_cli.py -v`
...
Run: `uv run pytest tests/test_web_discovery.py -v`
...
Run: `uv run pytest tests/test_web_write.py -v`
