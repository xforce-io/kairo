...
输出锚点：现网实测 provider 会把过程旁白与正文**粘在同一行**（例如「先查本地纪要写法…已定位 kairo 的规则。BRIEF: 实体建对方平台、不另起一套,群控先做事后分析、控制后置。」），与 digest.md 里过程句黏住 `# ` 标题是同一个毛病。因此不按位置取正文，改为唯一锚点规则：取最后一个 `BRIEF:`（全角冒号同接受）之后、到行尾为止的内容；没有锚点即失败，不猜哪一段是正文。首版按「整段就是正文」校验时，这类输出被记成 148 字超长，实际锚点内容只有 28 字。
...
| Unit | 校验与过期判定 | 空 / 缺锚点 / 锚点内容超 50 字 → `BriefError` 且不写 manifest；`brief_hash` 不匹配 → 判定过期 |
