...
前序项目 基准对比暴露根因：bench3 三源全是 stream、缺 corpus 层，所以校正不了 ASR 品牌名（「北港」未纠成「北港系统」）、给不出架构锚点 —— 不是推理弱，是缺一整类源 + 缺源分层语义。
...
source_classes:
  ...
  corpus:
    label: 基线
    hint: 权威参考资料；与观测冲突时以基线为准，用基线校正专名/术语(如 北港→北港系统)，并作术语权威基线。
