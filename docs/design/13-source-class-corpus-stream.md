...
示例课题 基准对比暴露根因：bench3 三源全是 stream、缺 corpus 层，所以校正不了 ASR 品牌名（「北港」未纠成「北港系统」）、给不出架构锚点 —— 不是推理弱，是缺一整类源 + 缺源分层语义。
...
- 真实复测（可选，验证价值）：示例课题 白皮书转 md 后 `add --corpus` 进 bench3 再 step，看品牌名是否被校正、架构锚点是否补齐。
