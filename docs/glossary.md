...
| 授读 | 让 agent 只读所选材料：文件复制进临时工作集；所选目录通过 `read_dirs` 授权。 | — |
| 工作集 | agent 工作目录内受控、唯一的材料文件副本；必读与按需分目录。 | 材料沙箱 |
...
| Reader | 按平台读取 Data Source 的可替换能力（腾讯文档、企微、Notion、IMAP）；不是 spreadsheet 类别。 | 爬虫、导入器、文件类型 |
| 企微文档 | 企业微信文档平台 Reader，覆盖在线文档、在线表格、智能表格、智能文档。 | 微信文档、企微表格 |
| 邮件检索 | Project Data Source 的一种：按查询条件只读检索邮箱，结果为 Markdown。 | 邮箱同步、邮件 Ref |
