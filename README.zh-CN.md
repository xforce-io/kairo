...
`[provider.codex] model` 会传给 `codex exec -m`，钉死 kairo run/step 用的模型，不跟随 `~/.codex/config.toml` 的默认模型。未配置则不传 `-m`。`reasoning_effort` 传给 `codex exec -c model_reasoning_effort=…`。`[agent] timeout_s` 是 digest / compose / 其它 CLI agent 的默认超时（调用未显式传 timeout 时）；未配置则 600 秒。`kairo run --ref` 之后的机器 note 固定走 `grok` + `--reasoning-effort low`，超时用 `[agent] note_timeout_s`（默认 120 秒），不跟随 auto/Codex，也不占用 digest 超时。`[agent] https_proxy` / `no_proxy` 会注入 CLI agent 子进程（grok/codex/claude），即使 serve 父进程已 unset 代理变量。
