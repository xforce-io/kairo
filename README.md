...
- **Synthesis output**: `understanding.md` (fact layer), capped at 20,000 Unicode characters, automatically compacted toward 12,000 during ordinary processing. New digests are processed in batches of up to 24,000 characters and 16 items (an oversized single digest gets its own batch). Each successful batch commits its fold progress; an oversized candidate gets at most one budget revision. Historical details remain in digests; failures preserve the last successful version. Digest/Compose grant reads through a materials catalog instead of putting source text in the prompt.
...
- **blocked states**: source/conversion reasons (`no-asr`, `asr-failed`, `convert-failed`, `missing-source`), `manual-edit`, `provider-failed`, Compose protections (`compose-degraded`, `compose-provenance-invalid`, `compose-migration-required`, `compose-over-budget`), and the digest shrink guard (`digest-degraded`). Provider failures and understanding capacity failures can be retried with ordinary Run; legacy capacity blocks no longer require repeated manual re-step. Manual edits, invalid provenance and shrink protection still require review. Ordinary Run does not clear `digest-degraded`. Failed candidates preserve the previous document or digest. Oversized legacy `compose-degraded` is observed and recovered as `compose-migration-required`.
